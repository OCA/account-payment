...
class PaymentTransaction(models.Model):
    ...
    payment_product_id = fields.Many2one(
        comodel_name="payment.icon", string="Used payment method"
    )

    def _compute_reference(self, provider_code, prefix=None, separator="-", **kwargs):
        """Override of `payment` to ensure that Worldline requirement for
    ...
    def _worldline_create_checkout_session_get_payload(self):
        ...
        payment_method = self.provider_id._get_provider_payment_method(
            self.provider_code
        )
        payload = {
            ...
            "order": {
                ...
                # required to create a token and for some redirected payment methods
                "customer": {
                    "billingAddress": {
        ...
        if payment_method.code in const.REDIRECT_PAYMENT_METHODS:
            ...
            for payment_icon in provider.payment_icon_ids:
                if not payment_icon.worldline_code:
                    continue
                wordline_codes.append(
                    const.PAYMENT_METHODS_MAPPING[payment_icon.worldline_code]
                )

            if wordline_codes:
                payload["hostedCheckoutSpecificInput"]["paymentProductFilters"] = {
                    "restrictTo": {
                        "products": wordline_codes,
                    },
                }
        return payload
    ...
    def _worldline_create_checkout_session(self):
        ...
        :rtype: dict
        """
        payload = self._worldline_create_checkout_session_get_payload()
        _logger.info(
    ...
    def _send_payment_request(self):
        ...
        if not self.token_id:
            raise UserError(
                _("Worldline: ") + _("The transaction is not linked to a token.")
            )
    ...
    def _process_notification_data(self, notification_data):
        ...
        payment_icon = self.env["payment.icon"]._get_from_wordline_code(
            payment_method_code, mapping=const.PAYMENT_METHODS_MAPPING
        )
        self.payment_product_id = payment_icon.id if payment_icon else False
        ...
        if not status:
            raise ValidationError(
                _("Worldline: ") + _("Received data with missing payment state.")
            )
        ...
        if status in const.PAYMENT_STATUS_MAPPING["pending"]:
            ...
            if status in const.PAYMENT_STATUS_MAPPING["cancel"]:
                ...
                _logger.info(
                    "Received data with invalid payment status "
                    "(%(status)s) for transaction with reference %(ref)s.",
        ...
        token = self.env["payment.token"].create(
            {
                ...
                "payment_details": pm_data.get("card", {}).get("cardNumber", "")[
                    -4:
                ],  # Padded with *
