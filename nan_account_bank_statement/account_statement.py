...
class account_bank_statement_split_line_wizard(osv.osv_memory):
    ...
    def action_split(self, cr, uid, ids, context):
        ...
        if wizard.amount:
            self.pool.get('account.bank.statement.line').split_line(
                cr, uid, context['active_id'], wizard.amount, context)
        return {
...
class account_bank_statement(osv.osv):
    ...
    _columns = {
        'total_amount': fields.function(
            _total_amount, method=True, string='Total Amount'),
    }

    def _attach_file_to_statement(
        self, cr, uid, file_contents, statement_id, attachment_name, file_name,
        context=None
    ):
        """
...
class account_bank_statement_line(osv.osv):
    ...
    _columns = {
        'data_ids': fields.one2many(
            'account.bank.statement.line.data', 'line_id', 'Related Data',
            readonly=True,
            help='Data related to the line, usually extracted/imported from a '
                 'file provided by the bank or another partner.'),
    ...
    def reconcile_search(
        self, cr, uid, ids, context, default_maturity_date=None, max_days=None
    ):
        """
        Tries to find the matching move line to reconcile using line's
        'search_by' field.

        It returns a dictionary where keys are the line identifiers and values
        are booleans indicating if
        ...
        for line in self.browse(cr, uid, ids, context):
            ...
            # Get all move lines already added for reconciliation in the
            # statement
            ...
            for statement_line in line.statement_id.line_ids:
                if statement_line.voucher_id and statement_line.id != line.id:
                    reconciled_move_line_ids += [
                        x.move_line_id.id for x in
                        statement_line.voucher_id.line_ids]

            # Get extra information attached to the line
            data = self.pool.get(
                'account.bank.statement.line.data').load_to_dictionary(
                    cr, uid, line.id, context)

            maturity_date = default_maturity_date or line.date
            line_ids = self.search_line2reconcile_by_imported_line(
                cr, uid, line, data, reconciled_move_line_ids, maturity_date,
                max_days, context)

            if line_ids:
                reconcile_line = self.pool.get(
                    'account.move.line').browse(cr, uid, line_ids[0], context)
                ...
                voucher_id = self.pool.get('account.voucher').create(cr, uid, {
                    ...
                    'partner_id': (
                        reconcile_line.partner_id and
                        reconcile_line.partner_id.id),
                    'journal_id': line.statement_id.journal_id.id,
                    'currency_id': line.statement_id.currency.id,
                    'account_id': (
                        reconcile_line.credit > 0.0 and
                        line.statement_id.journal_id.default_credit_account_id.
                        id or
                        line.statement_id.journal_id.default_debit_account_id.
                        id),
                ...
                for reconcile_line in self.pool.get(
                    'account.move.line'
                ).browse(cr, uid, line_ids, context):
                ...
                self.write(cr, uid, [line.id], {
                    ...
                    'partner_id': (
                        reconcile_line.partner_id and
                        reconcile_line.partner_id.id or False
                    ),
                    'type': account_type,
                }, context)
        ...
        if 'reference' not in data:
            if line.search_by != 'all':
                raise osv.except_osv(
                    _('Search by reference error'),
                    _('You cannot search by reference because it seems this '
                      'line has not been imported from a bank statement file. '
                      'The system expected a "reference" key in the line with '
    ...
    def _get_vats(self, cr, uid, line, data, context):
        if 'vat' not in data:
            if line.search_by != 'all':
                raise osv.except_osv(
                    _('Search by VAT error'),
                    _('You cannot search by VAT because it seems this line '
                      'has not been imported from a bank statement file. The '
                      'system expected a "vat" key in the line with amount '
    ...
    def _get_invoice_numbers(self, cr, uid, line, data, context):
        if 'invoice_number' not in data:
            if line.search_by != 'all':
                raise osv.except_osv(
                    _('Search by invoice error'),
                    _('You cannot search by invoice number because it seems '
                      'this line has not been imported from a bank statement '
                      'file. The system expected an "invoice_number" key in '
                      'the line with amount %(amount).2f in statement '
                      '%(statement)s.') % {
                        'amount': line.amount,
                        'statement': line.statement_id.name,
                        })
    ...
    def _get_invoice_origins(self, cr, uid, line, data, context):
        if 'invoice_origin' not in data:
            if line.search_by != 'all':
                raise osv.except_osv(
                    _('Search by invoice error'),
                    _('You cannot search by invoice origin because it seems '
                      'this line has not been imported from a bank statement '
                      'file. The system expected an "invoice_origin" key in '
                      'the line with amount %(amount).2f in statement '
                      '%(statement)s.') % {
                        'amount': line.amount,
                        'statement': line.statement_id.name
    ...
    def search_line2reconcile_by_imported_line(
        self, cr, uid, line, data, reconciled_move_line_ids, maturity_date,
        max_date_diff, context
    ):
        """
        ...
        if line.search_by in ('reference_and_amount', 'all'):
            for reference in self._get_references(
                cr, uid, line, data, context
            ):
                move_lines = (
                    self._find_entry_to_reconcile_by_line_ref_and_amount(
                        cr, uid, line, reference, reconciled_move_line_ids,
        ...
        if not move_lines and line.search_by in ('invoice_number', 'all'):
            for invoice in self._get_invoice_numbers(
                cr, uid, line, data, context
            ):
                move_lines = self._find_entry_to_reconcile_by_invoice_number(
                    cr, uid, invoice, reconciled_move_line_ids, context)
                if move_lines:
                    break

        # Search by invoice origin
        if not move_lines and line.search_by in ('invoice_origin', 'all'):
            for invoice in self._get_invoice_origins(
                cr, uid, line, data, context
            ):
                move_lines = self._find_entry_to_reconcile_by_invoice_origin(
                    cr, uid, invoice, reconciled_move_line_ids, context)
                if move_lines:
                    break

        if not move_lines and line.search_by in ('bank_statement', 'all'):
            move_lines = self._find_entry_to_reconcile_by_bank_statement(
                cr, uid, line, reconciled_move_line_ids, context)

        # Search by line amount.
        if not move_lines and line.search_by in ('amount', 'all'):
            move_lines = self._find_entry_to_reconcile_by_line_amount(
                cr, uid, line, reconciled_move_line_ids, maturity_date,
                max_date_diff, context)

        # Search unreconciled payment orders by amount.
        if not move_lines and line.search_by in ('payment_order', 'all'):
            payment_order = (
                self._find_payment_order_to_reconcile_by_line_amount(
                    cr, uid, line, reconciled_move_line_ids, maturity_date,
                    max_date_diff, context)
            )
            ...
            if payment_order:
                for line_id in payment_order.line_ids:
                    ...
                    voucher_id = self.pool.get(
                        ...
                        'date': line.date,
                        'type': (
                            line_id.amount < 0.0 and 'payment' or 'receipt'),
                        'partner_id': (
                            reconcile_line.partner_id and
                            reconcile_line.partner_id.id
                        ),
                        ...
                        'account_id': (
                            reconcile_line.credit > 0.0 and
                            line.statement_id.journal_id.
                            default_credit_account_id.id or
                            line.statement_id.journal_id.
                            default_debit_account_id.id
                        ),
                        'period_id': line.statement_id.period_id.id,
                        'company_id': (
                            line.statement_id.journal_id.company_id.id),
                    ...
                    self.copy(cr, uid, line.id, {
                        ...
                        'amount': (
                            payment_order.type == 'receivable' and
                            line_id.amount or -line_id.amount
                        ),
                        'voucher_id': voucher_id,
                        'type': account_type,
                    }, context)
                # Remove current line because all payment order lines have
                # already been created
                self.pool.get('account.bank.statement.line').unlink(
                    cr, uid, [line.id], context)
    ...
    def _get_default_partner_account_ids(self, cr, uid, context=None):
        ...
        for prop in self.pool.get('ir.property').browse(
            cr, uid, property_ids, context
        ):
    ...
    def _get_nearest_move_line(self, lines, maturity_date, max_date_diff=None):
        ...
        maturity_date_timestamp = time.mktime(
            time.strptime(maturity_date, '%Y-%m-%d'))
        for line in lines:
            line_date = line.date_maturity or line.date
            if line_date:
                line_timestamp = time.mktime(
                    time.strptime(line_date, '%Y-%m-%d'))
    ...
    def _find_entry_to_reconcile_by_invoice_number(
        self, cr, uid, number, reconciled_move_line_ids, context
    ):
        invoice_ids = self.pool.get('account.invoice').search(
            cr, uid, [('number', 'ilike', number)], context=context)
        domain = [
    ...
    def _find_entry_to_reconcile_by_invoice_origin(
        self, cr, uid, origin, reconciled_move_line_ids, context
    ):
        invoice_ids = self.pool.get('account.invoice').search(
            cr, uid, [('origin', 'ilike', origin)], context=context)
        domain = [
            ('account_id.reconcile', '=', True),
            ('account_id.type', 'in', ['receivable', 'payable']),
            ('invoice', 'in', invoice_ids),
    ...
    def _find_entry_to_reconcile_by_bank_statement(
        self, cr, uid, line, reconciled_move_line_ids, context
    ):
        ...
        lines = []
        for statement in self.pool.get('account.bank.statement').browse(
            cr, uid, ids, context
        ):
            account_ids = (
            ...
            if self.pool.get('res.currency').is_zero(
                cr, uid, statement.currency,
                statement.total_amount - line.amount
            ):
                for move_line in statement.move_line_ids:
                    if move_line.reconcile_id:
                        # If any of the lines of the statement has been
                        # reconciled it means this is not
    ...
    def _find_entry_to_reconcile_by_line_ref_and_amount(
        self, cr, uid, line,
        reference, reconciled_move_line_ids, maturity_date, max_date_diff,
        context=None
    ):
        domain = [
        ...
        return self._find_entry_by_amount(
            cr, uid, domain, line.amount, maturity_date, max_date_diff,
            context)

    def _find_entry_by_amount(
        self, cr, uid, domain, amount, maturity_date, max_date_diff, context
    ):
        if amount >= 0:
        ...
        line_ids = self.pool.get('account.move.line').search(
            cr, uid, domain, context=context)
        if line_ids:
            lines = self.pool.get('account.move.line').browse(
                cr, uid, line_ids, context)
            line_id = self._get_nearest_move_line(
                lines, maturity_date, max_date_diff)
            if line_id:
                return [line_id]
        return []

    def _find_entry_to_reconcile_by_line_vat_number_and_amount(
        self, cr, uid, line, vat, reconciled_move_line_ids, maturity_date,
        max_date_diff, context=None
    ):
        """
        Searchs for a non-conciled entry given the partner vat number of the
        line and amount.
        ...
        if not partner:
            partner = self._find_partner_by_line_vat_number(
                cr, uid, vat, context)
        ...
        if partner:
            ...
            return self._find_entry_by_amount(
                cr, uid, domain, amount, maturity_date, max_date_diff, context)

        return []

    def _find_entry_to_reconcile_by_line_amount(
        self, cr, uid, line,
    ...
    def _find_payment_order_to_reconcile_by_line_amount(
        self, cr, uid, line,
        reconciled_move_line_ids, maturity_date, max_date_diff, context=None
    ):
        """
        ...
        if 'payment.order' not in self.pool.obj_list():
            return None
        ...
        #
        query = """
                SELECT payment_line.order_id, SUM(payment_line.amount_currency)
                    AS total
                FROM payment_line
                INNER JOIN account_move_line
                    ON (payment_line.move_line_id = account_move_line.id)
        ...
        if len(res) == 1:
            ...
            payment_order = self.pool.get(
                'payment.order').browse(cr, uid, res[0][0], context)
...
class account_bank_statement_line_data(osv.osv):
    ...
    _columns = {
        'line_id': fields.many2one(
            'account.bank.statement.line', 'Statement Line', required=True,
            ondelete='cascade'),
        'key': fields.char('Key', size=256),
        'value': fields.char(
            'Value', size=256, required=False, readonly=False, help=''),
    }

    _sql_constraints = [
        ('data_line_key_value_uniq', 'unique(line_id,key,value)',
    ...
    def load_to_dictionary(self, cr, uid, line_id, context):
        ids = self.search(
            cr, uid, [('line_id', '=', line_id)], context=context)
        data = {}
...
class account_bank_statement_line_rule(osv.osv):
    ...
    _columns = {
        'company_id': fields.many2one(
            ...
            help='Company in which this rule should apply.'),
        'sequence': fields.integer(
            'Sequence', required=True,
            help='Rules will be applied in the order defined by this Sequence '
                 'and will stop in the first matching rule.'),
        'key': fields.char(
            'Key', size=256, required=True,
            help='Key in statement line data that should match the given '
                 'Expression.'),
        'expression': fields.char(
            'Expression', size=500,
            help='If the value of the given Key contains this Expression, '
                 'Account and Partner will be used for that statement line.'),
        'account_id': fields.many2one(
            'account.account', 'Account',
            domain="[('type','!=','view'),('company_id','=',company_id)]",
            ondelete='cascade',
            help='Account to be used if expression matches.'),
        'partner_id': fields.many2one(
            'res.partner', 'Partner', ondelete='cascade',
            help='Partner to be used if expression matches'),
    }
    ...
    def _check_company(self, cr, uid, ids):
        for rule in self.browse(cr, uid, ids):
            if (
                rule.account_id and
                rule.company_id != rule.account_id.company_id
            ):
                raise osv.except_osv(
                    _('Company Check Error'),
                    _("Company for account %(account)s does not match rule's "
                      "company for rule %(rule)s.") % {
