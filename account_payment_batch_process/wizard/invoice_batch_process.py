...
class InvoiceCustomerPaymentLine(models.TransientModel):
    ...
    def _onchange_amount(self):
        self.check_amount_in_words = self.invoice_id.currency_id.amount_to_text(
            self.receiving_amt
        )
...
class AccountRegisterPayments(models.TransientModel):
    ...
    def _compute_pay_total(self):
        for rec in self:
            rec.total_pay_amount = sum(line.paying_amt for line in rec.invoice_payments)
    ...
    cheque_amount = fields.Float("Check Amount", required=True, default=0.00)
    total_pay_amount = fields.Monetary("Total Invoices", compute="_compute_pay_total")
    total_customer_pay_amount = fields.Monetary(
    ...
    def default_get(self, pfields):
        ...
        if any(
            INV_TO_PARTN[inv.type] != INV_TO_PARTN[invoices[0].type] for inv in invoices
        ):
            raise UserError(
                _(
        ...
        if any(inv.currency_id != invoices[0].currency_id for inv in invoices):
            raise UserError(
                _(
                    "In order to pay multiple invoices at once, they"
        ...
        if "batch" in context and context.get("batch"):
            ...
            if INV_TO_PARTN[invoices[0].type] == "customer":
                ...
                dict_val = {"invoice_customer_payments": lines, "is_customer": True}
                rec.update(dict_val)
            else:
                for inv in invoices:
                    dict_line = {
                    ...
                    lines.append((0, 0, dict_line))
                dict_val = {"invoice_payments": lines, "is_customer": False}
                rec.update(dict_val)

        else:
            # Checks on received invoice records
            if any(
                INV_TO_PARTN[inv.type] != INV_TO_PARTN[invoices[0].type]
                for inv in invoices
            ):
                raise UserError(
                    _(
    ...
    def get_payment_batch_vals(self, group_data=None):
        ...
        res = {
            ...
            "invoice_ids": [(4, int(inv), None) for inv in list(group_data["inv_val"])],
        ...
        p_model = self.env.ref("account_check_printing.account_payment_method_check")
    ...
    def make_payments_customer(self):
        ...
        for paym in self.invoice_customer_payments:
            if paym.receiving_amt > 0:
                paym.payment_difference = paym.balance_amt - paym.receiving_amt
                ...
                if partner_id in data:
                    ...
                    p_method_pay = (
                        paym.payment_method_id.id if paym.payment_method_id else False
                    )
                    dict_data_part = {
                    ...
                    dict_data_part_inv = {
                        str(paym.invoice_id.id): {
                            ...
                            "writeoff_account_id": paym.writeoff_account_id
                            and paym.writeoff_account_id.id
                            or False,
                        }
                    }
                    ...
                    if self.communication:
                        memo = "".join(
                            [self.communication, "-", str(paym.invoice_id.number)]
                        )
                    else:
                    ...
                    amount_word = self.currency_id.amount_to_text(paym.receiving_amt)
                    dict_writeoff_account_id = (
                        paym.writeoff_account_id.id
                        if paym.writeoff_account_id
                        else False
                    )
                    dict_data_upd = {
                        partner_id: {
                            ...
                            "inv_val": {
                                str(paym.invoice_id.id): {
                                    ...
                                    "handling": paym.handling,
                                    "payment_difference": paym.payment_difference,
                                    "writeoff_account_id": dict_writeoff_account_id,
                                }
                            },
                        }
                    }
        ...
        for paym in self.invoice_payments:
            if paym.paying_amt > 0:
                ...
                if partner_id in data:
                    ...
                    dict_val_up = {str(paym.invoice_id.id): paym.paying_amt}
                    data[partner_id]["inv_val"].update(dict_val_up)
                else:
                    # Build memo value
                    if self.communication:
                        p_memo = [self.communication, "-", str(paym.invoice_id.number)]
                        memo = "".join(p_memo)
                    else:
                        memo = str(paym.invoice_id.number)
                    # Calculate amount in words
                    ...
                    dict_val_up = {
                        partner_id: {
                            ...
                            "total_check_amount_in_words": amount_word,
                            "memo": memo,
                            "inv_val": {str(paym.invoice_id.id): paym.paying_amt},
                        }
                    }
    ...
    def make_payments(self):
        ...
        if self.is_customer:
            ...
            for paym in self.invoice_customer_payments:
                if not paym.payment_method_id:
                    raise ValidationError(
                        _("Verification Failed! Payment Method" " must be indicate.")
                    )
            ...
            for paym in self.invoice_payments:
                if (paym.balance_amt - paym.paying_amt) < 0.00:
                    raise ValidationError(
                        _("Verification Failed! Amount" " not must be bigest.")
                    )

            data = self.make_payments_supplier()

        # Update context
        dict_val = {"group_data": data}
        context.update(dict_val)
        # Making partner wise payment
        payment_ids = []
        for p_index in list(data):
            val_ap = self.env["account.payment"]
            payment = val_ap.with_context(context).create(
                self.get_payment_batch_vals(group_data=data[p_index])
            )
        ...
        result["domain"] = f"[('id','in', {payment_ids})]"
        return result
