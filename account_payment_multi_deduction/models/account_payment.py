...
class AccountPayment(models.Model):
    ...
    def _create_payment_entry_multi_deduct(self, amount):
        ...
        debit, credit, amount_currency, currency_id = MoveLine.with_context(
        ...
        if self.payment_difference:
            for deduct in self.deduction_ids:
                ...
                debit_wo, credit_wo, amount_currency_wo, currency_id = MoveLine.with_context(
                    date=self.payment_date
                )._compute_amount_fields(
                    deduct.amount, self.currency_id, self.company_id.currency_id
                )
                ...
                if counterpart_aml["debit"] or (
                    writeoff_line["credit"] and not counterpart_aml["credit"]
                ):
...
class AccountRegisterPayments(models.TransientModel):
    ...
    def _prepare_payment_vals(self, invoices):
        ...
        values["deduction_ids"] = [
            (0, 0, {"account_id": x.account_id.id, "amount": x.amount, "name": x.name})
