...
class TestPaymentMultiDeduction(SavepointCase):
    ...
    def test_1_one_invoice_payment(self):
        ...
        ctx = {
            "active_ids": [self.cust_invoice_1.id],
            "active_id": self.cust_invoice_1.id,
            "active_model": "account.invoice",
        }
        PaymentWizard = self.env["account.payment"]
        view_id = "account_payment_multi_deduction." "view_account_payment_invoice_form"
    ...
    def test_2_two_invoices_register_payment(self):
        ...
        view_id = (
            "account_payment_multi_deduction." "view_account_payment_from_invoices"
        )
        with Form(PaymentWizard.with_context(ctx), view=view_id) as f:
        ...
        self.assertRecordValues(
            move_lines,
            [
                {"account_id": bank_account.id, "debit": 600.0, "credit": 0.0},
                {
                    "account_id": self.account_expense.id,
                    "name": "Expense 2",
                    "debit": 200.0,
