...
class AccountPaymentRegister(models.TransientModel):
    ...
    writeoff_analytic_dist_ids = fields.Many2many(
        comodel_name="account.analytic.distribution.model",
        relation='writeoff_analytic_model_rel',
        string="Analytic Distribution",
    )
    deduct_analytic_account_id = fields.Many2one(
        comodel_name="account.analytic.account",
           relation='deduct_analytic_model_rel',  
        compute="_compute_default_analytic",
    )
    deduct_analytic_dist_ids = fields.Many2many(
        comodel_name="account.analytic.distribution.model",
        relation='deduct_analytic_model_rel',  
        compute="_compute_default_analytic",
    )
    ...
    def _compute_default_analytic(self):
        ...
        moves_without_tax = move_lines.filtered(
            lambda l: l.account_id.account_type not in ("payable", "receivable")
        ...
        for rec in self:
            ...
            rec.deduct_analytic_dist_ids = default_tag
    ...
    def _update_vals_deduction(self, moves):
        ...
        self.writeoff_analytic_dist_ids = default_tag
    ...
    def _create_payment_vals_from_wizard(self):
        ...
        if (
            ...
            payment_vals["write_off_line_vals"]["analytic_distribution_ids"] = [
                (6, 0, self.writeoff_analytic_dist_ids.ids)
            ]
        # multi deduction
        elif (
