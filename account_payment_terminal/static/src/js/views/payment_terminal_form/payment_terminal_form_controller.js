odoo.define(
    "account_payment_terminal.OCAPaymentTerminalFormController",
    function (require) {
        "use strict";
                ...
                if (transaction.success) {
                    ...
                    self._confirm_payment(transaction)
                        ...
                        .catch(() => {
                            self._show_error(
                                _lt(
                                    "Payment Successful but couldn't confirm it in odoo"
                                )
                            );
                        });
            ...
            _poll_for_transaction_status: function () {
                ...
                var timerId = setInterval(() => {
                    ...
                    this._proxy_call("status_json", {params: status_params}, 1000)
                        ...
                        .fail(() => {
                            self._show_error(
                                _lt("Error querying terminal driver status")
                            );
                            clearInterval(timerId);
                        });
                }, 1000);
            ...
            start: function () {
                ...
                return this._super.apply(this, arguments).then(function () {
                    return self._get_payment_info().then(function (payment_info) {
                        self.oca_payment_terminal_id =
                            payment_info.oca_payment_terminal_id;
                        var data =
                            self._get_payment_terminal_transaction_start_data(
                                payment_info
                            );
                        self._proxy_call(
                            ...
                            .fail(function () {
                                self._show_error(
                                    _lt("Error starting payment transaction")
                                );
                                return false;
                            });
                    });
