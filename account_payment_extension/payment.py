...
class PaymentMode(orm.Model):
    ...
    _columns = {
        ...
        'require_bank_account': fields.boolean(
            ...
            help='Ensure all lines in the payment order have a bank account '
                 'when proposing lines to be added in the payment order.'),
        'require_received_check': fields.boolean(
            'Require Received Check',
            help='Ensure all lines in the payment order have the Received '
                 'Check flag set.'),
        'require_same_bank_account': fields.boolean(
            'Require the Same Bank Account',
            help='Ensure all lines in the payment order and the payment mode '
...
class ResPartnerBank(orm.Model):
    ...
    def create(self, cr, uid, vals, context=None):
        if vals.get('default_bank') and vals.get('partner_id') and \
                vals.get('state'):
            sql = "UPDATE res_partner_bank " \
                  "SET default_bank='0' " \
                  "WHERE partner_id=%i AND default_bank='1' " \
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if 'default_bank' in vals and vals['default_bank']:
            partner_bank = self.pool['res.partner.bank'].browse(cr, uid, ids)[0]
            ...
            cr.execute(sql)
        return super(ResPartnerBank, self).write(cr, uid, ids, vals,
                                                 context=context)
...
class PaymentOrder(orm.Model):
    ...
    def _get_period(self, cr, uid, context=None):
        try:
            ...
            # installed it will create the new field and try to update existing
            # records (even if there are no records yet) So we must ensure no
            # exception is thrown, otherwise the module can only be installed
    ...
    def _payment_type_name_get(self, cr, uid, ids, field_name, arg,
                               context=None):
        result = {}
    ...
    _columns = {
        'type': fields.selection([('payable', 'Payable'),
        ...
        # The field name is necessary to add attachement documents to payment
        # orders
        'name': fields.function(_name_get, method=True, type="char", size=64,
                                string="Name"),
        'create_account_moves': fields.selection(
            ...
            help='Indicates when account moves should be created for order '
                 'payment lines. "Bank Statement" will wait until user '
                 'introduces those payments in bank a bank statement. "Direct '
                 'Payment" will mark all payment lines as payied once the '
                 'order is done.'),
    ...
    def cancel_from_done(self, cr, uid, ids, context=None):
        ...
        self.pool['account.move'].button_cancel(cr, uid, remove,
                                                context=context)
        self.pool['account.move'].unlink(cr, uid, remove, context=context)
            ...
            if t['state'] in ('draft', 'cancel'):
                ...
                raise orm.except_orm(
                    ...
                    _('You cannot delete payment order(s) which are already '
                      'confirmed or done!'))
        result = super(PaymentOrder, self).unlink(cr, uid, unlink_ids,
                                                  context=context)
        return result
            ...
            for line in order.line_ids:
                ...
                if line.account_id and line.account_id.currency_id and \
                    ...
                    if company_currency_id == line.account_id.currency_id.id:
                        ...
                        amount_cur = currency_obj.compute(
                            ...
                            line.account_id.currency_id.id, amount, context=ctx)
                    val['amount_currency'] = amount_cur
                partner_line_id = move_line_obj.create(
                    cr, uid, val, context=context, check=False)
                ...
                move_line_obj.create(
                    ...
                    {'name': (line.move_line_id and line.move_line_id.name or
                     ...
                     'state': 'valid'}, context=context, check=False)
                if line.move_line_id and not line.move_line_id.reconcile_id:
                    ...
                    # Check if payment line move is already partially reconciled
                    # and use those moves in that case.
                    if line.move_line_id.reconcile_partial_id:
                        partial = line.move_line_id.reconcile_partial_id
                        for rline in partial.line_partial_ids:
                            lines_to_reconcile.append(rline.id)
                    else:
                        lines_to_reconcile.append(line.move_line_id.id)
                    amount = 0.0
                    for rline in move_line_obj.browse(
                            cr, uid, lines_to_reconcile, context=context):
                ...
                payment_line_obj.write(
                    cr, uid, [line.id], {'payment_move_id': move_id}, context)
            # Post the move
...
class PaymentLine(orm.Model):
    ...
    def _auto_init(self, cr, context=None):
        cr.execute("SELECT column_name "
                   ...
                   "WHERE table_name = 'payment_line' and column_name='type'")
        update_sign = bool(cr.fetchone())
    ...
    _columns = {
        ...
        'type': fields.related(
            'order_id', 'type', type='selection',
            selection=[('payable', 'Payable'),
                       ('receivable', 'Receivable')], readonly=True,
    ...
    def onchange_move_line(self, cr, uid, ids, move_line_id, payment_type,
        ...
        if move_line_id:
            line = self.pool['account.move.line'].browse(cr, uid, move_line_id)
            if line.name != '/':
                res['value']['communication'] = (res['value']['communication'] +
                                                 '. ' + line.name)
            res['value']['account_id'] = line.account_id.id
            if context.get('order_id'):
                payment_order = self.pool['payment.order'].browse(
