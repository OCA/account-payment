...
class TestPaymentAcquirer(TransactionCase):
    def setUp(self) -> None:
        ...
        self.list_acquirers = list(self.acquirers)
    ...
    def test_get_allowed_acquirers_blank_option(self):
        ...
        result = self.env["payment.acquirer"].get_allowed_acquirers(
            self.list_acquirers, order_id=self.order.id
        )
        ...
        # If there's no order
        result = self.env["payment.acquirer"].get_allowed_acquirers(self.list_acquirers)
        self.assertListEqual(
            result,
