...
class PaymentProvider(models.Model):
    ...
    def _ogone_get_api_url(self, api_key):
        ...
        if self.state == "enabled":
            api_urls = {
                "hosted_payment_page": "https://secure.ogone.com/ncol/prod"
            ...
            if self.env.context.get("refund", False):
                api_urls[
                    "directlink"
                ] = "https://secure.ogone.com/ncol/prod/maintenancedirect_utf8.asp"
        else:  # 'test'
            api_urls = {
                "hosted_payment_page": "https://ogone.test.v-psp.com/ncol/test"
                "/orderstandard_utf8.asp",
            ...
            if self.env.context.get("refund", False):
                api_urls[
                    "directlink"
                ] = "https://ogone.test.v-psp.com/ncol/test/maintenancedirect_utf8.asp"
    ...
    def _ogone_generate_signature(self, values, incoming=True, format_keys=False):
        ...
        :param bool format_keys: Whether the keys must be formatted as uppercase, dot-separated
                                 strings to comply with Ogone APIs.
                                 This must be used when the keys are formatted as
                                 underscore-separated strings to be compliant with
                                 QWeb's `t-att-value`.
    ...
    def _ogone_make_request(self, payload=None, method="POST"):
        ...
        """
        self.ensure_one()
        if payload.get("OPERATION", False) == "RFD":
            self = self.with_context(refund=True)

        url = self._ogone_get_api_url("directlink")
        try:
            response = self._ogone_request(method, url, payload, timeout=60)
            response.raise_for_status()
        except requests.exceptions.ConnectionError as e:
            _logger.exception("unable to reach endpoint at %s", url)
            raise ValidationError(
                _("Ogone: Could not establish the connection to the API.")
            ) from e
        except requests.exceptions.HTTPError as e:
            _logger.exception("invalid API request at %s with data %s", url, payload)
            raise ValidationError(
                _("Ogone: The communication with the API failed.")
            ) from e
        return response.content

    def _ogone_request(self, method, url, payload, timeout):
        return requests.request(method=method, url=url, data=payload, timeout=timeout)
