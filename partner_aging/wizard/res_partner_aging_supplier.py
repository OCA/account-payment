    ...
    def execute_aging_query(self, age_date=False):
        ...
        query = """
                SELECT aml.id, aml.partner_id as partner_id,
                 ai.invoice_user_id as salesman, aml.date as date,
                aml.date as date_due, ai.name
                ...
                type = 'payable')
                AND aml.date <= '%s'
                AND ai.state = 'posted' AND (ai.invoice_payment_state != 'paid' OR
                    aml.full_reconcile_id IS NULL) AND ai.type = 'in_invoice'
                GROUP BY aml.partner_id, aml.id, ai.name, days_due,
                ai.invoice_user_id, ai.id
                UNION
                SELECT aml.id, aml.partner_id as partner_id,
                ai.invoice_user_id as salesman, aml.date as date,
                aml.date as date_due, ai.name as
