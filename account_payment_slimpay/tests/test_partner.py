...
class PartnerTC(TransactionCase):
    ...
    def partner_data(self, **kwargs):
        ...
        data = {'firstname': 'F', 'lastname': u'C', 'country_id': fr.id}
        data.update(kwargs)
        return data

    def test_zip(self):
        'Zip must have 5 figures when country is France'
        p = self.env['res.partner'].create(self.partner_data(zip=u'67000'))
    ...
    def test_zip_no_country(self):
        ...
        self.env['res.partner'].create(
            self.partner_data(zip=u'0', country_id=False))

    def test_slimpay_checks(self):
        '`slimpay_checks` must return a dict of errors like {attr: error_msg}'
        partner_model = self.env['res.partner']
        self.assertEqual(
            partner_model.slimpay_checks(self.partner_data(zip=u'67000')),
            {})
        self.assertEqual(
            partner_model.slimpay_checks(self.partner_data(zip=u'0')),
            {'zip': u'Incorrect zip code (should be 5 figures)'})
