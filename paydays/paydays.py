...
class account_payment_term(osv.osv):
    ...
    def _check_payment_days(self, cr, uid, id, context=None):
        days = self.read(cr, uid, id, ['payment_days'], context)[
            0]['payment_days']
        if days is False:
            return True
    ...
    _columns = {
        'payment_days': fields.char(
            'Payment days', size=11,
            help="If a company has more than one payment days in a month you "
                 "should fill them in this field and set 'Day of the Month' "
                 "field in line to zero. For example, if a company pays the "
                 "5th and 20th days of the month you should write '5-20' "
                 "here."),
    }
    _constraints = [
        (
            _check_payment_days,
            'Error: Payment days field format is not valid.', ['payment_days']
        )]

    def compute(self, cr, uid, id, value, date_ref=False, context=None):
        result = super(account_payment_term, self).compute(
