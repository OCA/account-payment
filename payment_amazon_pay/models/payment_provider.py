...
class PaymentProvider(models.Model):
    ...
    amazon_pay_private_key = fields.Text(
        string="Private Key",
        required_if_provider="amazon_pay",
    ...
    def _get_supported_currencies(self):
        ...
        return self.env["res.currency"].with_context(active_test=False).search(
            [("name", "in", supported_currency_codes)]
        )
    ...
    def _amazon_pay_build_headers(self, method, api_path, payload_text="", idempotency_key=None):
        self.ensure_one()
        ...
        if self.state == "test" and self.amazon_pay_sandbox_simulation_code:
            headers_to_sign["x-amz-simulation-code"] = self.amazon_pay_sandbox_simulation_code
        ...
        private_key = serialization.load_pem_private_key(
            self.amazon_pay_private_key.encode("utf-8"),
            password=None,
        )
    ...
    def _amazon_pay_request(self, method, endpoint, payload=None, idempotency_key=None):
        self.ensure_one()
        payload = payload or {}
        payload_text = "" if method.upper() == "GET" else json.dumps(
        ...
        if response.status_code >= 400:
            message = data.get("reasonDescription") or data.get("message") or response.text
            raise ValidationError(_("Amazon Pay API error: %s") % message)
    ...
    def _amazon_pay_prepare_checkout_payload(self, tx):
        ...
        result_url = url_join(base_url, f"/payment/amazon_pay/return?reference={tx.reference}")
        cancel_url = url_join(base_url, f"/payment/amazon_pay/cancel?reference={tx.reference}")
        payload = {
            "storeId": self.amazon_pay_store_id,
            "webCheckoutDetails": {
                "checkoutResultReturnUrl": result_url,
                "checkoutCancelUrl": cancel_url,
                "checkoutReviewReturnUrl": result_url,
            },
            "paymentDetails": {
            ...
            "merchantMetadata": {
                ...
                "merchantStoreName": (self.company_id.name or self.name or "Store")[:50],
                "customInformation": tx.reference,
            },
            "productType": "PayOnly",
            "scopes": ["name", "email", "phoneNumber", "billingAddress"],
        }
        if tx.partner_id.email:
            payload["buyer"] = {"email": tx.partner_id.email}
        if self.amazon_pay_soft_descriptor:
            payload["paymentDetails"]["softDescriptor"] = self.amazon_pay_soft_descriptor[:16]
        return payload
    ...
    def _amazon_pay_get_checkout_session(self, checkout_session_id):
        self.ensure_one()
        return self._amazon_pay_request("GET", f"checkoutSessions/{checkout_session_id}")
